...
public class DialogCreator {
	 ...
	 * 
	 * @param dialogName der name des Dialogs
	 * @return der Dialog
	 */
	...
	public void intintialtheDialogcreator() {
		...
		// Dialogebene 1
		DialogCommand kaninchen1 = new DialogCommand("Wo bin ich?",
				"du bist mir gefolgt und in den Brunnen gefallen.");
		DialogCommand kaninchen2 = new DialogCommand(
				"Was soll ich als nächstes tun?",
				"Geh nach Westen und Sammle die Blaue beere ein.");
		DialogCommand kaninchen3 = new DialogCommand("Wer bist du?",
				"Ich bin nicht der Schmunzelhase und habe keine Zeit.");
		...
		// Dialogebene 1.2
		DialogCommand kaninchen2u1 = new DialogCommand(
				"OK das werde ich! Was macht die BlaueBeere?",
				"Sie Lässt dich schrumpfen aber...");
		kaninchen2.insertDialog(kaninchen2u1);
		// Dialogebene 1.2.1
		DialogCommand kaninchen2u1u1 = new DialogCommand("Aber was?",
				"Nutze sie nur wenn du sie brauchst sie hält nur...");
		kaninchen2u1.insertDialog(kaninchen2u1u1);
		// Dialogebene 1.2.1.1
		DialogCommand kaninchen2u1u1u1 = new DialogCommand(
				"Hält nur Was??? Rede in ganzen Sätzen.",
				"Hält nur so Lange wie du brauchst um einen Raum zu durchqueren!");
		kaninchen2u1u1.insertDialog(kaninchen2u1u1u1);

		// Dialogebene 1.3
		DialogCommand kaninchen3u1 = new DialogCommand(
				"Jetzt mal ehrlich wer bist du?",
				"na was wohl ich bin ein Kaninchen und Weiss...");
		kaninchen3.insertDialog(kaninchen3u1);
		// Dialogebene 1.3.1
		DialogCommand kaninchen3u1u1 = new DialogCommand("Der Essenkehrer!!",
				"Mann bist du Doof!!");
		DialogCommand kaninchen3u1u2 = new DialogCommand("Das Weiße Kaninchen",
				"Richtig!!");
		...
		dialogs.put("Hutmacher", new DialogCommand(
				"Hallo Allis ich habe dich erwartet."));
		dialogs.put("Katze", new DialogCommand(
				"Hihihi na da bist ja Allis hat ja lange gedauert"));
		dialogs.put(
				"Feuergeist",
				new DialogCommand(
						"Hui ich bin der Geist, der im Kamin Wohnt..wo kommst du den her."));
		dialogs.put("Schneemann", new DialogCommand(
				"Ein eisiges Hallo ***Oh du Fröhliche****"));
	}

	/**
	 * Die methode die asusgibt ob der Dialog vorhanden ist
	 * @param secondWord der name des Dialogs
